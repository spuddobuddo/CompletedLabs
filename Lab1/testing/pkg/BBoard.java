...
public class BBoard {		// This is your main file that connects all classes.
	...
	public void loadUsers(String inputFile) throws FileNotFoundException {
		...
		while(scan.hasNextLine())
		{
			...
			User bruh = new User(split[0], split[1].trim());
			userList.add(bruh);
		}
	...
	public void login(){
		...
		while (uCheck == false)
		{
			...
			usr[0] = sc.nextLine();
			if (usr[0].equals("Q") || usr[0].equals("q"))
			{
			...
			for (int i = 0; i < userList.size(); i++)
			{
				if (userList.get(i).check(usr[0], usr[1]) == true)
				{
	...
	*/
	...
	private void addTopic(){
		...
		messageList.add(new Topic(currentUser.getUsername(), subj, bod, mesC));
		mesC++;
	}
		...
		while (bool)
		{
			...
			if (mesId != -1)
			{
				for (int i = 0; i < messageList.size(); i++)
				{
					...
					if (mesId-1 == messageList.get(i).getId())
					{
						System.out.print("Body: ");
						String body = sc.nextLine();
						System.out.println("check");
						Reply rep = new Reply(currentUser.getUsername(), ("Re: "+messageList.get(i).getSubject()), body, mesC);
						messageList.get(i).addChild(rep);
						messageList.add(rep);
						mesC++;
						bool = false;
						break;
